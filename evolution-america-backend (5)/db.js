...
async function seedIfEmpty() {
  ...
  db.data.schools.forEach((s) => {
    ...
    if (s.courseCover === undefined) { s.courseCover = null; migrated = true; }
    s.users.forEach((u) => {
    ...
    s.classes.forEach((c) => {
      c.students.forEach((st) => {
        ...
        if (st.paymentStatus === undefined) { st.paymentStatus = 'em_dia'; migrated = true; }
        if (st.certificateIssuedAt === undefined) { st.certificateIssuedAt = null; migrated = true; }
      });
    });
  });

...
const { requireSchoolAuth, requireSchoolAdmin } = require('../lib/auth-middleware');
...
// PUT /api/schools/branding { logoDataUrl, email, phone, cpfCnpj }
// Autoatendimento da própria escola — restrito à direção (admin) da escola.
// Nota importante: cadastrar ou excluir uma ESCOLA (não a marca dela) é ação
// exclusiva do Painel Evolution — ver routes/owner.js. Uma escola nunca cria
// outra escola por aqui.
router.put('/branding', requireSchoolAdmin, async (req, res) => {
  const db = req.db;
  const { logoDataUrl, email, phone, cpfCnpj } = req.body || {};
